...
/**
 * Direct-charge Checkout for JORD-as-seller flows (supplies marketplace).
 * No connectedAccountId, no application_fee — funds land in the platform's
 * own Stripe balance. Buyer can be asked for a shipping address in the
 * Checkout UI by passing `collectShipping: true`.
 */
async function createDirectCheckoutSession({
  amountCents, productName, productDescription, productImageUrls,
  buyerEmail, metadata, successUrl, cancelUrl,
  collectShipping = false, quantity = 1,
}) {
  assertLive();
  const params = {
    mode: 'payment',
    ...
    line_items: [{
      price_data: {
        ...
        product_data: {
          name: productName,
          ...(productDescription ? { description: productDescription } : {}),
          ...(Array.isArray(productImageUrls) && productImageUrls.length ? { images: productImageUrls.slice(0, 8) } : {}),
        },
      },
  ...
  if (collectShipping) {
    // US-only for now — adding more countries here is a one-line change.
