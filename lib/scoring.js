...
/**
 * Vegas: two pairs of two. Each pair's two NET scores combine into a 2-digit
 * number (lower score first → 4 + 6 = 46). Lowest combined per hole wins by
 * the difference between the two numbers. Optional "birdie flip": when the
 * opposing pair makes a net birdie or better on the hole, you must put your
 * HIGHER score first (a 4 + 6 becomes 64 instead of 46).
 *
 ...
 *
 * Returns a 2-row leaderboard: one row per pair, with `total` = net point
 * margin (positive when this pair is ahead). `score` is gross points scored.
 */
function buildVegas(entries, fmt, settings) {
  const flipBirdie = !!(settings && settings.flip_birdie);
  // Group by team — Vegas needs exactly 2 teams of 2 players.
  const teams = {};
  ...
  if (T.length !== 2 || T.some(t => t.members.length !== 2)) {
    // Not a valid Vegas configuration — return an empty board with a hint
    // so the UI can prompt the organizer to set up 2 pairs of 2.
    return { format: fmt.id, scoreType: 'vegas', rows: [],
      note: 'Vegas requires exactly 2 pairs of 2 players (assign team labels A and B in the player list).' };
  }
  // Holes both teams have scored fully (all 4 players have a value).
  ...
  const pointsA = [0, 0]; // [own points, opponent points] for each team — kept symmetric
  const thru = [0, 0];
  ...
  for (const h of sortedHoles) {
    ...
    if (nets[0].some(s => s == null) || nets[1].some(s => s == null)) continue;
    // Par for the hole (any member has it) — used for birdie detection.
    const par = T[0].members[0].net[h]?.par || 0;
    const birdied = nets.map(ns => ns.some(n => n <= par - 1)); // net birdie OR better
    const numA = combine(nets[0], flipBirdie && birdied[1]);
    const numB = combine(nets[1], flipBirdie && birdied[0]);
    const diff = Math.abs(numA - numB);
    if (numA < numB) pointsA[0] += diff;
    else if (numB < numA) pointsA[1] += diff;
    thru[0] += 1;
    thru[1] += 1;
  }
  ...
  const rows = T.map((t, i) => {
    ...
    return {
      ...
      courseHandicap: null,
      thru: thru[i],
      total: thru[i] ? own - opp : null, // signed margin (positive = ahead)
      score: thru[i] ? own : null,
    };
  });
